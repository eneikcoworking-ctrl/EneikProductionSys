...
*Опровержение:* найти в журнале случай, когда `BLOCKED_BY_REVIEW` снялся без вмешательства человека раньше
часа. Есть — пункт смягчается.

---

### Итог

Тридцать пять пунктов, все со своим образцом, замером, заслоном и опровержением. Порядок работы задан
