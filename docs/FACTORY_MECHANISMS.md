...
*Живое, 8 сентября 2026, Codex:* частично снят один путь подъёма строк: `julesSessions(null)` теперь отвечает через `count()` и `countByStatus(...)` (`2cc3cfe`), а проектный путь больше не читает все сессии, а берёт только `findByTaskIdIn(...)` для задач проекта (`75b0318`). Проверка: `docker run --rm -v /opt/EneikProductionSys:/workspace -w /workspace -v /root/.m2:/root/.m2 maven:3.9-eclipse-temurin-17 mvn -q -Dtest=SystemStatusServiceTest test` — пройдена. Механизм не закрыт: в `SystemStatusService` остаются другие `findAll()`-пути.
