...
*Делать.*
1. **Завести карточку.** Единица права на производство — свободный слот у ограничения (внешняя квота Jules),
   и он обязан **возвращаться событием завершения**, а не проверяться таймером.
