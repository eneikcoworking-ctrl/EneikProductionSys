...
### 20. Личность задачи выводится из работы, а не чеканится заново · `PERSISTENCE_SNAPSHOT` (D010) · **лекарство**

Пункт 19 даёт прибор — он покажет размножение. Этот пункт делает размножение **невозможным**, и он же
единственный, который не добавляет ещё одной проверки.

*Что обнаружено рядом.* `ProjectFlowService:3477` снабжает заголовок задачи компилятора обрывком
идентификатора заявки, и комментарий говорит зачем: одинаковые заголовки — «нормальное, законное явление» —
ложно поднимали тревогу о дубликатах. То есть о размножении знали и сделали задачи **на вид разными, чтобы
сигнализация замолчала**. По корпусу это `PRINCIPLED_INTEGRITY` (D012): местная починка, удовлетворяющая
букве правила и убивающая его смысл. Отсюда и невидимость класса.

*Суть решения.* Сегодня личность задачи чеканится заново на каждом обороте: одна работа — тридцать одна
личность. Личность обязана быть **функцией работы**:

    identity(задача компилятора) = f(проект, упорядоченное множество заявок, роль)

Создание становится «найти или создать по ключу», а ключ — **уникальный индекс в хранилище**, не проверка в
сервисе.

*Почему именно в хранилище, а не сторожем.* Это ответ на вопрос об идеальном взаимодействии. Сторож в одном
сервисе держит только тех, кто через него идёт; в заявки пишут двадцать механизмов, в задачи пятнадцать.
**Ограничение в общем состоянии держит всех двадцать, включая тот, который напишут завтра.** Инвариант живёт
в состоянии, а не в добросовестности участника — и потому не зависит от того, назван ли уже хозяин.

*Чем это лучше очевидных альтернатив:*
* «Запретить восстановлению сбрасывать» — ломает его законную работу.
* «Ужесточить сторож» — сторож и так работал: он честно видел отсутствие активной задачи, потому что её
  снесли. Ужесточение не помогает там, где состояние действительно выглядит готовым.
* «Ограничить число задач на заявку» — назначенная константа, запрещённая законом 8, и лечит след.

*Побочные выигрыши, ради которых это и стоит делать:*
1. **Повтор становится видимым.** Сегодня тридцать одна попытка — это тридцать одна задача, у каждой свой
   счётчик, и ни по одной не видно, что работа повторяется. С единой личностью попытки копятся **на одной
   задаче**, и предел `A_max` наконец меряет то, что называет.
2. **Тревога о дубликатах оживает.** Обрывок идентификатора в заголовке становится не нужен: дубликаты
   делаются невозможными, а не маскируются. Снять этот суффикс — часть пункта.
3. **Не нужен хозяин состояния, чтобы это заработало.** Пункты 1 и 2 остаются главными, но этот работает и
   до них.

*Осторожность, которую нельзя опустить.* Иногда ту же заявку законно компилировать заново — после
окончательного провала прежней попытки. Тогда в ключ входит поколение, и оно сдвигается **только** по
записанному поглощающему вердикту, а не по всякой неудаче. Иначе поколение станет счётчиком, и мы вернём
размножение под другим именем.

*Заслон:* при существующей задаче компилятора для того же множества заявок повторное создание **не
порождает второй строки** — проверяется на уровне хранилища, а не сервиса. Плюс: попытки накапливаются на
одной задаче.

*Опровержение:* вызвать создание дважды подряд из двух разных мест кода и посчитать строки в базе. Две
строки — пункт не сделан, как бы он ни был назван.

---

### Итог

Двадцать пунктов, все со своим образцом, замером, заслоном и опровержением. Порядок работы задан
