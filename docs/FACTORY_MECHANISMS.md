...
**`ProjectFactoryService`** — разворачивает рабочее место проекта: репозиторий, доступы, воркспейс.
*Связи:* зовёт `ProjectWorkspaceFactoryService` и провижининг GitHub/Linear | вызывает его один `ProjectFlowService`
*Ценность:* без него принятое требование некуда класть — нет ни репозитория, ни ветки, ни доступа
*Комментарий:* **ядро по позиции, периферия по частоте**: срабатывает один раз на проект, но до него поток вообще не начинается. Единственный вызывающий — это хорошо: точка входа одна
*Философия:* `PART_WHOLE_OWNERSHIP` (D004) — форма: **сильная**; один владелец на одно создание, разделять нечего
...
**`OperationalPolicyService`** — решает, разрешено ли действие оркестрации: отправка, восстановление
границы, ревью, аудит покрытия.
*Связи:* читает `OperationalFlowCoreService.build` | спрашивают `ContinuousOrchestrationService`, `AutoMergeService`, `ClientDeliverableReadinessService`, наблюдатель
*Ценность:* один запрет вместо запрета в каждом вызывающем; иначе правило «когда можно» разъедется по семи местам
*Комментарий:* **ядро**. Это самый частый отказ в живом логе. Здесь же лежит уже исправленный дефект, который стоит помнить: отказ ссылался на состояние `BLOCKED_BY_REVIEW`, которого нет в `GLOBALLY_BLOCKING_STATES`, то есть **называл причиной то, что ничего не запрещало**. Теперь `stateBlocks` знает действие, и `denialReason` называет тот запрет, который сработал
*Философия:* `TRUTH_STATUS_TABLE` (D012) — форма: **сильная**; отказ обязан называть свой действующий конъюнкт, и после правки называет
...
**`GateOrchestrator`** — прогоняет проверки качества и спецификации над задачей и пишет их исход в
`task_gate_logs`.
*Связи:* собирает все `GateCheck` | зовут `ClaimService`, `DeliveredWorkJudgmentService`; проверки — `DesignExcellenceGate`, `VerificationEvidenceGate` и другие
*Ценность:* единственное место, где «прошло» получает основание; без него слияние опирается на слово исполнителя
*Комментарий:* **ядро**. Здесь живёт образцовый разбор пустого квантора: `allMatch` на пустом списке истинна, а у восьми ролей из тринадцати нет ни одной применимой проверки — такая задача записывалась как прошедшая **все** проверки, не пройдя ни одной. Знаменатель обязан называться и у булевых величин, не только у долей
*Философия:* `FALSIFICATION_HARNESS` (D008) — форма: **сильная после правки**; опровержение: задача роли без единой применимой проверки, помеченная «прошла»
...
**`EmsMetricsService`** — считает свод показателей потока для панели: стадии, время, доли.
*Связи:* читает `julesSessionRepository`, `prReviewRepository`, задачи и заявки | зовут `ProjectFlowService`, `TechnicalLeadCompiler`, `DoctrineVerdictLayer`
*Ценность:* показать поток одной картиной, а не семью запросами
*Комментарий:* **периферия**: поток не держит, только смотрит. Но 849 строк на один класс-считалку — это тот же размер, за который разделены два ядерных класса; предписания 3–4 распространяются и сюда, только с меньшей срочностью
*Философия:* `ANTI_MIRROR_TELEMETRY` (D013) — форма: **не мерено**; сводный показатель обязан быть проверяем против механизма, который считает то же самое, а такой сверки здесь нет
...
*Философия:* `PLANNING_CONSISTENCY` (D004) — **сильная**. Опровержение: найти второй цикл запуска.
...
**`LogScope`** — ставит в MDC признак, к какому проекту относится строка лога (`SYSTEM` либо id проекта).
*Связи:* зовут 13 механизмов, среди них `ContinuousOrchestrationService`, `AutoMergeService`, `OpsAuditorService`
*Ценность:* без него журнал проекта неотделим от системного, и весь разбор происшествий идёт вслепую
*Комментарий:* **периферия по действию, ядро по наблюдаемости**. Сегодняшний разбор семичасового простоя целиком оперся на то, что строки помечены проектом: без этого замер «после 13:08:05 аккаунты не появляются» был бы невозможен
*Философия:* `LEVEL_OF_ABSTRACTION_LOCK` (D010) — форма: **сильная**; область видимости объявлена явно, а не выводится из текста строки
*Опасность:* `clear()` обязан звать тот же поток, что и `project()`. Планировщик переиспользует потоки, и незакрытая область приписывает чужие строки проекту
...
Двадцать четыре пункта, все со своим образцом, замером, заслоном и опровержением. Порядок работы задан
