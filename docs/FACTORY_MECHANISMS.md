...
*Замер.* Шесть аккаунтов из семи стоят `enabled = false`. Оператор их не выключал.
...
*Что НЕ установлено.* Кто выключил. Две версии, различить нечем: миграция V19 переносила аккаунты из
`jules_configs` и **копировала поле `enabled` оттуда** (`SELECT … api_key, enabled, …`), либо это вызов
`PATCH /api/accounts/{id}`. Ни один путь не оставляет записи — см. пункт 21.
...
Двадцать три пункта, все со своим образцом, замером, заслоном и опровержением. Порядок работы задан
