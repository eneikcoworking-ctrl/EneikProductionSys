...
*Комментарий:* **ядро.** Предписания 17 и 34 закрыты как единый механизм. Закон 8 (`BELIEF_UPDATE_LEDGER`, D007),
`CAUSAL_PROCESS_TRACE` (D013, `ELVIN_GOLDMAN_06`) и `PRINCIPLED_INTEGRITY` (D012):
1. Собственная временная метка `finalizing_since` (миграция V139) фиксирует точный момент перехода в `finalizing`.
   Устранена категориальная ошибка замера от чужих меток (`lastCompileDispatchedAt` / `createdAt`), приводившая
   к преждевременному снятию защиты (п. 34).
2. Активное продление аренды (`renewFinalizingLeases` в `REQUIRES_NEW`) на этапах поиска PR и разбора плана:
   работающий исполнитель непрерывно подтверждает свой захват и никогда не лишается его (п. 17).
3. Динамический расчет предела аренды (`calculateEffectiveLeaseDuration`): срок выводится из медианы наблюдённых
   длительностей завершения `FINALIZING_DURATION` в `DefectJournalRepository` с 10-кратным запасом.
4. Обход проектов оптимизирован: использует `findByStatusOrderByCreatedAtDesc(ProjectStatus.active)`.
*Философия:* `BELIEF_UPDATE_LEDGER` (D007), `CAUSAL_PROCESS_TRACE` (D013), `PRINCIPLED_INTEGRITY` (D012) — **сильная**.
Опровержение заслонено: работник с продлением аренды не сносится; заявка со старой отправкой не снимается преждевременно.
...
4. **Темп порождения дубликатов (`checkDuplicateGenerationVelocity`):**
   ...
   - Единица процесса: для задач компиляции (где механизм V137 оживляет одну строку по `contentKey`) темп измеряется по числу сессий Jules за окно и `wishlist.compileAttempts`; для задач среза — по числу созданных строк в окне. Учитываются все попытки, включая терминальные (`done`, `failed`).
...
*Живое, 11 сентября 2026, Antigravity (L2): такт 4 — SystemStatusService доведён до идеала целиком.*
...
*Живое, 11 сентября 2026, Antigravity (L2): такт 6 — ProjectEventLogRetentionService целиком (пункт 5 очереди, Раздел XLIV).*
