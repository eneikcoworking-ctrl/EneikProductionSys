...
### 39. Тянущий механизм построен, соблюдается — и привязан не к тому концу · `TELEOSEMANTIC_FEEDBACK` (D011) + `FALSIFICATION_HARNESS` (D008)
...
*Дефект — в том, к чему верёвка привязана.* Замер по всему коду: `startExecution` вызывается **ноль раз** в
`ProjectFlowService`, `JulesDispatchService` и `ClaimService`. То есть **отправка работы в Jules через
...
*Связь с предписанием 38, и это одна болезнь.* Раз ничего не тянет, отправка **толкает**; а в толкающем пути
порядок выбора аккаунта односторонне разжалует того, кто отказал, и вернуться ему нечем. Верёвка, стоящая на
входе, — естественное место, где нагрузка делится: она решает, **сколько** впускать, а порядок решает,
**кому** отдать. Сейчас первого нет, и второе работает без противовеса.
...
Тридцать девять пунктов, все со своим образцом, замером, заслоном и опровержением. Порядок работы задан
