...
*Философия:* `PART_WHOLE_OWNERSHIP` (D004) — форма: **не мерено**; владелец расписания объявлен, а его ёмкость с числом таймеров нигде не сверена
...
**Замер 2026-09-06: описаны все 149 классов-механизмов, неназванных ноль.** Проверка — перечислить в коде
все `@Service`/`@Component`/`@RestController`/`@Configuration` и поискать каждое имя в этом файле.
