...
Из 58 файлов каталога `models/persistence` **шесть** несут методы, не являющиеся аксессорами; остальные —
данные и механизмами по демаркации не являются. Замер: подсчёт публичных методов, чьё имя не начинается на
`get`/`set`/`is`. Трое из шести уже описаны (`TaskEntity`, `WishlistEntity`, `JulesSessionEntity`).
