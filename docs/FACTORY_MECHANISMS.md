...
### 22. Ревью не повторяется никогда, если первая попытка не оставила следа · `TRUTH_STATUS_TABLE` (D012) · **СРОЧНО: держит поток**

*Замер.* Конвейер стоит 49 минут при открытых PR и свободном аккаунте. В журнале: `Poka-yoke: PR review
fallback was already attempted for task … ; automatic retry is disabled` — семь отказов за сорок минут. Три
задачи ревью созданы и не уходят.

*Механизм.* `JulesDispatchService.reviewFallbackTargetsEverAttempted` собирает множество целей, по которым
ревью **когда-либо** запускали, и повтор пропускается. Выход один: у цели должен быть счётчик повторов по
пустому вердикту больше нуля и ниже предела. **Замер целей: счётчик не установлен ни у одной.** Значит выхода
нет, и эти PR не будут отревьюированы автоматически **никогда**.

*Суть дефекта.* Метка «пробовали» ставится независимо от того, чем попытка кончилась. Попытка, давшая вердикт,
и попытка, не давшая ничего, помечаются **одинаково**, а различить их обязан счётчик, которого падающий путь
не выставляет. Одно поле стоит за два состояния — тот же класс, что «не смог проверить» ≡ «проверил».

*Делать.* Различить три исхода вместо двух: **ревью прошло и дало вердикт** / **ревью прошло и вердикта не
дало** / **ревью не запускалось**. Второй исход обязан быть виден и обязан включать повтор в пределах бюджета;
сегодня он неотличим от первого. Счётчик выставлять на **том** пути, который завершается без вердикта, а не
ожидать его от постороннего.

*Заслон:* попытка ревью, завершившаяся без вердикта, оставляет цель пригодной для повтора в пределах бюджета.
На нынешнем коде тест обязан краснеть.

*Опровержение:* довести ревью до завершения без вердикта и посмотреть, уйдёт ли вторая попытка.

*Немедленно, без кода:* выставить счётчик повтора трём целям — это вернёт их в очередь законным путём. Правка
живых данных, поэтому только по слову оператора.

---

### Итог

Двадцать два пункта, все со своим образцом, замером, заслоном и опровержением. Порядок работы задан
