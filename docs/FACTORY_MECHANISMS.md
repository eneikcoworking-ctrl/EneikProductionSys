...
публичных методов, чьё имя не начинается на `get`/`set`/`is`. Трое из шести уже описаны (`TaskEntity`,
`WishlistEntity`, `JulesSessionEntity`).
...
**Полный знаменатель, перемерян 6 сентября — 522 единицы, записей 125.**
...
`@Service|@Component|@RestController|@Controller|@Repository|@Configuration`.
    ...
    слой                                          всего   записей
    бэкенд, бины Spring                             187       95
    бэкенд, классы без аннотации                    162       27
...
Записей у сервисов **89, а не 127**. Двенадцать объявленных исключений — типы результатов и исключения —
записи действительно не требуют. Остальные **38 не описаны**, и среди них несущие:
