# Archive - full working plan, 2026-08-19 to 2026-08-21

This is the complete text as it stood before the 2026-08-21 compression. Nothing was deleted: the
working plan was reduced to what is needed to act, and everything else - the derivations, the
session-by-session verdict records, the Gemini removal, the TOC and Six Sigma material - was moved
here whole. It is kept because the measurements in it were paid for with real time and cannot be
re-derived cheaply.

The working plan is `LIVE_PRODUCT_PLAN_2026-08-19.md`. If a claim here disagrees with it, the
working plan is newer.
...
| O-17 | the forced-unblock budget - the ONLY thing that closes a stale session and frees the WIP slot - is reset by the classifier verdict that our own unblock message provokes, so mu never decreases and the session is never closed | Measured live 2026-08-21: task `47aa70cb` has held IMPLEMENTING for **373 minutes against a 90-minute SLA** without producing a PR. Its unblock counter runs `1 of max 2`, `2 of max 2`, `1 of max 2`, `2 of max 2` - 15:41:09, 15:42:00, 15:42:47, 15:43:47 - because a `PROGRESSING` verdict resets `forcedUnblockAttempts` to 0. The two OTHER reset sites in the same file reset on artefact evidence (a merged PR, a new commit); this one resets on a judgement about conversation that our own nudge stirred up. `honorDavidsonProgressEvidence` vetoes the clock separately, with no overall bound either. Flow Core correctly refuses to dispatch anything new while the slot is held, so this one session blocks L-4 | factory |
...
| L-5 | Verify V109's collapse | Needs two observations of the *same* commit; `main` has moved between every pair so far | **done, verified live** - 18 real observations, 6 of them on `74af88ee`, collapsed to 13 draws; posterior mean 0.066667 = Beta(1,14) exactly - **подтверждено живьём 2026-08-21** |
| L-6 | §7 policy predicate | Its precondition is met (§13.4, §13.5). L-9 removed the *identification* half of §7's inversion; this is the *subordination* half - the rule that decides what may run while the constraint is open | **built 2026-08-21, not yet observed live** - see *L-6, as built* below - **built, tested, deployed 2026-08-21** (`e6cb928`), running at `shadow` |
...
**O-17, as built, and what it cost to find.** The operator chose a ceiling on session age over a
consecutive-veto counter, because the counter needs a new column and a migration, and the store currently
has no rollback snapshot at all (see O-6). Declared as `jules.davidson-veto-ceiling-minutes`, default 360 -
three times the Davidson close window.

Below the ceiling the charity is untouched: a session whose own writing shows real reasoning gets its trust
window back exactly as before. Above it the veto stops restoring the forced-unblock budget and **closes
nothing itself** - it merely stops holding the circuit breaker open, so the existing closure mechanism can
fire. Session age is monotone and the ceiling is fixed, so the veto terminates. Same shape as L-2: the bound
falls through to machinery that already exists rather than inventing a second one, and the log says it
closed on the bound rather than on evidence about the session.

Both directions are pinned by tests. The one that matters more is the second: past-the-ceiling behaviour is
the fix, but below-the-ceiling behaviour is the thing a later edit could silently destroy by removing the
charity instead of bounding it.

**The stuck task was cleared by hand, on the operator's explicit instruction**, by setting task `47aa70cb`
to `failed` through `PATCH /internal/tasks/{id}` - terminal, so the factory's OWN path released the claim
and closed the session (15:57:31), rather than any hand-editing of session rows. The slot was taken by new
work within seconds and `done` moved 143 -> 144. This is an operator intervention of exactly the kind the
seven points exist to remove; it is recorded here so it is not mistaken for the factory having recovered on
its own.
