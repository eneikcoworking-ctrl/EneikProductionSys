...
**9. Порядок отчёта обратен порядку работы.** Замер 2026-09-06: с момента записи этого алгоритма из 30
коммитов **22 — механизмы**, то есть работа шла по шагу 4 каждый такт без пропуска. А оператор каждые
полчаса видел отчёт о фабрике и справедливо спросил, занимаюсь ли я механизмами вообще. Причина не в
дисциплине: **алгоритм задал порядок работы и не задал порядок отчёта**, и я докладывал в том порядке, в
каком выполнял — сперва `git`, Куайн и живое, а механизм в конце, если хватало места.

    отчёт открывается механизмом и образцом; числа фабрики идут последней строкой.

Живое состояние — одна строка: числитель, застой, отказы, ошибки. Если в нём ничего не изменилось, так и
писать: «без изменений». Разбор фабрики выносится наверх **только** когда она стоит или числитель двинулся.
Шаги 1–3 остаются обязательными к исполнению и перестают быть обязательными к пересказу: замер, никого не
удививший, места в отчёте не занимает.
