...
17. **🔴 СРОЧНО, ОСТАНОВКА ПОТОКА. Слияния прекратились. Требуется $\mathcal{L}_2$.**

    **Замер.** За последний час — ни одного слияния. Новые PR продолжают открываться и копятся открытыми;
    `AutoMergeService` в логе только связывает и сверяет PR, ни разу не сливает. Открытых ревью стало
    больше, слитых — не прибавилось.

    **Механика, установленная чтением кода.** После удаления обхода единственный путь к слиянию —
    `AutoMergeService` строка 227/229: `executeMerge(review)` вызывается либо для `chaotic`-задачи, либо
    когда `review.getDiffSummary()` содержит `APPROVAL_TOKEN` (`AutoMergeService.java:46`,
    `"CORE ARCHITECTURE VERIFIED. APPROVED."`). Токен берётся из вердикта ревью. Вердикт читается с ветки
    самого ревьюера (`JulesDispatchService.java:2913`, `parseReviewVerdictBatch(project, pr.headRef(), path)`),
    то есть слияния для его получения не требуется — замкнутого круга здесь нет.

    **Где стоит.** Основной ревьюер вердиктов не даёт: каждое ревью уходит в Jules-фолбэк
    (`JulesDispatchService.java:3366`), а фолбэк — это целая сессия на пакет, и на момент замера вердикта
    ещё не было («no valid verdict entry found ... attempt 1/3»). Сам Gemini при этом жив: в тот же период
    `OpsAuditorService` получил от него ответ. Значит дело не в ключе, а в том, почему путь ревью через
    Gemini не выбирается вовсе. **Это и есть точка, которую нужно найти и починить.**

    **Чего НЕ делать.** Возвращать `reconcileCleanOpenGitHubPullRequests` нельзя: именно он сливал всё
    подряд без ревью и без проверки на наличие кода — та самая засорённость репозитория, на которую указал
    оператор. Откат вернёт спам, а не поток.

    **Оговорка о причинности.** Прекращение слияний совпало по времени с удалением обхода, но счётчик
    слитых ревью двигался и после развёртывания правки, поэтому «удаление обхода — причина остановки»
    согласуется с наблюдениями, но не доказано. Доказанное: единственный оставшийся путь требует токена
    одобрения, и токен сейчас не появляется.
