...
**1. ОБРАЩЕНИЕ К ПАТТЕРНАМ КОРПУСА. Отдельный шаг, первый, обязательный.**
`grep` по `docs/philosopher-patterns` — по 86 файлам философов и по `03_PATTERN_STRENGTH.md`. Не по памяти,
не по названию, которое я помню: **грепом**, с показанной командой. Шаг обязан дать четыре вещи, и такт без
них не начинается:

    идентификатор образца     напр. MAYKL_DAMIT_11_KNOWLEDGE_FIRST_GATE, а не «по Даммиту»
    философ и его принцип     чей это образец и на каком published anchor он стоит
    сильная и слабая форма    из 03_PATTERN_STRENGTH.md, дословно
    опровержение образца      что наблюдать, чтобы образец оказался нарушен

Образец берётся **до** чтения кода. Взятый после — украшает вывод, взятый до — **задаёт, что мерить**. Это
проверяемо: в такте, где обращение было настоящим, команда грепа видна, и идентификатор образца попадает в
запись механизма и в сообщение коммита.

**2. Механизм назначает очередь** — те, у кого форма образца «слабая / не мерено / нарушена», — а не то, на
что я наткнулся.

**3. Условие опровержения названо до вывода**, вместе со второй объясняющей версией.
**4. Довести в этом же такте.** Дробить на непроверяемые движения запрещено.

**5. Перепроверить прежнее своё утверждение**, а не сегодняшнее.
