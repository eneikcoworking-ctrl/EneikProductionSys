...
**Открыто, замерено, законом не закрытое.** Заявка `b1c27ea9` возвращается восстановлением из `compiling` в
`pending` 4 раза за 15 минут, и на каждый заход создаётся **новая** задача компилятора — уже две. Числа
величины, убывающей на обороте, здесь нет: пока привязанный аккаунт отдыхает, задачи копятся. Это тот же круг
самозаказа, что и `delivery_never_reached_main`, только на уровне компиляции.
