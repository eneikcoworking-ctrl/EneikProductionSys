...
Такт — не список дел, а один законченный ход. Прежняя запись была списком из девяти шагов, наросшим
заплатками: каждый раз, когда я ошибался, я дописывал шаг. Это и есть муда, которую я ищу в фабрике, поэтому
список заменён на устройство.

**Цель такта:** довести **один** механизм до законченного состояния и оставить модель истинной. Не «пройти
шаги», а произвести одну вещь, которую можно предъявить.

**Выход такта:** одна законченная вещь и отчёт, по которому следующий работник может действовать.

---

## Три движения

**I. ВЗЯТЬ.** Предмет назначает **очередь**, а не то, на что я наткнулся: механизмы, у которых форма образца
стоит «слабая / не мерено / нарушена». Образец из корпуса берётся **до** чтения кода, грепом по
`docs/philosopher-patterns`, вместе с его обязательством доказательства и опровержением. Образец, взятый
после, украшает вывод; взятый до — задаёт, что мерить.
*Чему мешает:* тактам, где я разбирал случайную находку и не двигал очередь.

**II. ДОВЕСТИ.** Мерить обе стороны — код и живое, — и **до** вывода назвать две объясняющие версии и то
наблюдение, которое убьёт предпочитаемую. Довести до конца в этом же такте: запись с цепью механизмов и
ролью каждого звена, что делать, заслон, опровержение; если это код — заслон красный без правки, зелёный по
чистому коммиту, развёрнут и найден в живом образе. Дробить на движения, которые нельзя проверить,
запрещено.
*Чему мешает:* половинчатым правкам и предписаниям без заслона.

**III. СВЕРИТЬ СЕБЯ.** Одно **прежнее** моё утверждение перепроверяется против мира — не то, что я сделал
сейчас. Санитария только там, где замер стал ложью; ничего не стало — так и писать. Муда считается числом:
имя, возглавляющее две записи; строки не того рода, что файл; пункт, объявленный открытым и уже сделанный.
*Чему мешает:* тому, что мои ошибки ловил оператор, а не я.

---

## Четыре правила, действующие всё время

1. **У числа есть команда.** Утверждение — либо **замер** (называю команду), либо **вывод** (называю
   выкладку, две версии и убивающее наблюдение), либо **догадка** (говорю, что догадка). Не могу назвать
   команду — предложение не уходит.
2. **У проверки на отсутствие есть контрольная проба.** Искать заодно то, что заведомо есть. Не нашлось —
   сломан прибор, а не мир. Так `strings` и `unzip`, которых нет в образе, дважды говорили мне «нет».
3. **Ничто не удаляется, пока не показано, что оно есть в другом месте либо ложно.** Это же относится к
   сокращению: убрать пример — значит проверить, что он сохранён.
4. **Прежде чем вставить — грепнуть.** Муда дешевле всего там, где её не произвели: пять записей-дублей и
   два пункта с общим примером ловились одним грепом.

---

## Отчёт

    механизм и образец → что сделано → что осталось открытым

**Фабрики в отчёте нет.** Ни строкой, ни в конце, ни «без изменений». Она появляется **только** при одном из
двух событий: она **стоит**, либо **числитель двинулся** — и тогда это не строка в хвосте, а предмет такта,
и стоит первым.

Первая правка этого правила гласила «фабрика одной строкой» и оставляла её обязательной в каждом отчёте.
Оператор назвал это ложью, и назвал верно: я объявил изменение, а симптом остался. Обязательная строка есть
обязательный пересказ, сколько бы коротким он ни был. **Молчание о фабрике означает: ничего не изменилось.**

Шаги измерения обязательны к исполнению и **не обязательны к пересказу**: замер, никого не удививший, места
в отчёте не занимает.

**Испытание отчёта — `CONVERSATION_MAXIM` (D007), из корпуса, а не выдуманное.** Сильная форма: вывод
информативен, истинен, уместен и однозначен **для следующего работника**; названы минимальные поля
состояния, ссылка на свидетельство и следующее действие. Слабая — «отчёт длинный и подробный». Опровержение:
дать отчёт следующему работнику и спросить, что делать; **невозможность ответить и есть дефект**.

**Чего в такте нет намеренно.** Нет шага «доложить о выполнении шагов»: такт предъявляется вещью, а не
отчётом о процессе. Нет шага «улучшить алгоритм»: если такт не сработал, это видно по отсутствию вещи, и
чинится устройством, а не новым пунктом.
