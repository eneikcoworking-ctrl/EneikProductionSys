...
## 2026-09-12 02:21 UTC — Клод: пункт 31 до правки — запись требует двух поправок, и настоящий дефект глубже

Образец: `GROUPING_PROXIMITY_GATE` (D011, `03_PATTERN_STRENGTH.md:189`) — «связанные элементы измеримо ближе друг к другу,
чем к несвязанным… опровержение: измерить отношение расстояний»; индекс (`ENDI_KLARK_02_GROUPING_PROXIMITY_GATE`):
«Proof obligation: show spacing ratios between related-element clusters versus distance to the nearest unrelated element».

**Поправка 1. Второе разрешение в фабрике требуется — опровержение записи буквально выполнено.**
Запись говорит: «найти в задании ревьюеру дизайна хоть одну строку, требующую второго разрешения. Есть — пункт снимается».
Строка есть, но в другом механизме: `JulesDispatchService:684–692` для ролей `DesignExcellenceGate.UI_TAGS` требует
**два настоящих PNG** — `desktop-1440.png` и `mobile-375.png`, и без них задача не проходит заслон. Так что «никто не
смотрит на телефон» — неверно; смотрят, но **не на то**. Пункт не снимается, а переформулируется.

**Поправка 2. Заслон есть, подключён и срабатывает на настоящем потоке.** `GateOrchestrator` собирает проверки списком
(`List<GateCheck>`), `DesignExcellenceGate.stage()` — `IMPLEMENTATION_RESULT`, `supports` — только `BARCAN-TAG-03` и
`BARCAN-TAG-11`. Живой замер сейчас: у проекта **35** задач этих ролей (34 TAG-11, одна TAG-03) из 369 — заслон не висит
без дела.

**Настоящий дефект, и он в критерии, а не в отсутствии проверки.** `DesignExcellenceGate:95–121`: «отзывчивость» считается
выполненной, если **размеры файлов двух снимков просто различны** (вес 40); остальное — существование файлов (30) и «больше
килобайта» (30); проходной балл 70. То есть два PNG разного веса дают зачёт, а **внутрь снимков никто не смотрит**.
Наложение меню — отрицательное расстояние между прямоугольниками — заслон увидеть не может **по построению критерия**.
Это и есть обязательство образца, не выполненное: числа расстояний нет, есть число байт. Рядом второй дефект того же рода:
задание ревьюеру дизайна (`ProjectFlowService:6249–6259`) прямо предписывает «do NOT reject it solely for missing a second
resolution» и «be lenient by design» — и **ни один тест этот текст не закрепляет** (grep по `src/test` — ноль), то есть
формулировку можно менять без красного теста.
*Совет.* 1) Числу — быть: проверять геометрию (пересечение прямоугольников, отношение расстояний внутри группы к
расстоянию до чужой) по разметке отданного экрана либо по снимку с разбором, а не по весу файла. 2) Разделить предметы:
ревьюеру-человеку оставить вкус и снисходительность, а «телефон не сломан» сделать машинным заслоном в
`DesignExcellenceGate` — тогда «be lenient» перестанет быть дырой. 3) Закрепить текст задания тестом, иначе он снова
разойдётся с механизмом.
*Заслон (по записи, уточняю форму):* разметка с двумя пересекающимися прямоугольниками — отказ; и отказ при
`user-scalable=no`. *Опровергнет:* два снимка разного веса с наложенными меню проходят заслон.

**Закрываю открытый вопрос записи об отзывчивости продукта (запись: «не установлено»).** Облик продукта — 12 файлов
`.svelte`; приставки отзывчивости (`sm:`, `md:`, `lg:`) есть в **8** из 12 (`sm:px-`, `md:col-span-`, `lg:px-`,
`sm:flex-row`, `md:grid-cols-`), «чистых» `@media` — **0**. Мой первый замер дал ноль по всему продукту, но это была
**ошибка поиска**: я перечислил расширения без `.svelte` (контроль: тем же поиском слово `class` тоже дало ноль, а по
`.svelte` даёт 12). Итог: продукт отзывчив утилитарными приставками, а не медиазапросами; наложение меню, о котором
говорит оператор, происходит **несмотря** на объявленную отзывчивость — тем важнее машинная проверка геометрии.
**Чего не измерил:** `user-scalable=no` на живой странице перепроверить не удалось — продукт снесён по окну наблюдения
(`docker ps` — контейнеров продукта нет, снос в 02:13:25). Проверю при следующем подъёме.

## 2026-09-12 02:39 UTC — Клод: проверка bac28c8 (пункт 31) — держится по устройству, но на живом продукте заслон промолчит

**Держится.** Заведён `LayoutGeometryAuditService` (384 строки): настоящие прямоугольники (`BoundingBox` с `overlaps`,
`overlapArea`, `distanceTo`), попарная проверка пересечений, отношение расстояний внутри группы к чужим — то самое
обязательство `GROUPING_PROXIMITY_GATE` (D011), выраженное числом, а не вкусом. Запрет масштабирования ловится отдельно
(`user-scalable=no`, `maximum-scale=1`) и **возвращает отказ**, а не предупреждение. Заслон дизайна перестал давать 40
баллов за одно различие размеров: теперь требуется и различие, и пройденная геометрия (`:150`). Задание ревьюеру дизайна
переписано и **закреплено тестом** — `designReviewPrompt_explicitlySeparatesAutomatedMachineGates` утверждает, что в тексте
названы машинные заслоны, «1440px desktop», «375px mobile», «non-overlapping bounding» и «mobile viewport scalability»;
раньше текст не закреплял ничто. Множество путей расширено на `.svelte`, `.vue`, `.jsx`, `.tsx` — продукт на Svelte в
предмет попадает. Прогоны по её экрану: `LayoutGeometryAuditServiceTest` 8/8, `DesignExcellenceGateTest` 9/9.

**Остаток, и это та же ошибка рода, что мы закрыли в пунктах 25 и 28: неизвестное выдано за ответ.**
`auditBoxes` (`:247–250`): при **пустом** наборе прямоугольников возвращается `passed = true` с причиной
«no elements to audit». А прямоугольники берутся только из `data-rect`/`data-box`, `<rect>` и абсолютных inline-стилей.
Живой замер продукта: `data-rect`/`data-box`/`<rect>` — **0** файлов, `layout-check.json`/`layout.json` — **0**
(контроль: файлов `.svelte` — 12, тем же поиском достаются). Инструкции исполнителю выложить геометрию в коде нет:
единственное упоминание `layout.json` во всём `src/main` — сам фильтр путей заслона (`DesignExcellenceGate:204`).
Следствие: на настоящем PR продукта заслон прочитает `.svelte`, получит пустой набор, скажет «прошло» — и 40 баллов
снова будут выданы за одно различие веса снимков. Механизм верен, но предмета у него нет.
*Совет, две строки работы.* 1) Пустой набор — **не** «прошло»: отдельный исход «геометрия не выводима» (как `CANNOT_JUDGE`
в аудите согласованности), который не даёт 40 баллов и не отвергает задачу, а называет причину. 2) Связать требование с
заслоном: в том же задании, где уже требуются два PNG (`JulesDispatchService:684–692`), потребовать третий файл —
`layout-check.json` с прямоугольниками из того же прогона Playwright (`getBoundingClientRect`), по одному набору на
разрешение. Тогда геометрия появится в предмете, а не в намерении.
*Заслон:* разметка без геометрии — заслон не начисляет 40 и называет «геометрия не выводима»; PR с `layout-check.json`,
где два прямоугольника пересекаются, — отказ. *Опровергнет:* PR продукта с двумя снимками разного веса и без файла
геометрии проходит заслон с зачётом отзывчивости.

**Выкладка.** Образ — `a701efe` (23:49), коммит `bac28c8` — 02:37. **В репозитории, на фабрике нет.**
Незакрытым остаётся и мой замер `user-scalable=no` на живой странице: продукт снесён по окну наблюдения (снос 02:13:25),
проверю при следующем подъёме — теперь это стало проверяемо машиной, и сверить стоит именно на нём.

## 2026-09-12 03:01 UTC — Клод: проверка 5590315 — остаток пункта 31 закрыт; **в репозитории, на фабрике нет**

**Держится, и взято именно то, что было названо.**
1. **Третий исход стал третьим.** `GeometryVerdict.CANNOT_JUDGE` отделён от `FAILED`: при пустом наборе прямоугольников
   заслон **не начисляет 40 баллов** и пишет причину «responsive check cannot be judged: геометрия не выводима», но не
   объявляет столкновение. Столкновение и запрет масштабирования по-прежнему отвергают. Проход остался при 70, так что
   «не могу судить» стоит ровно столько, сколько стоит незнание, — ни зачёта, ни казни.
2. **Ключевая инверсия.** `layoutGeometryPassed` теперь инициализируется **false** (было `true`): сорок баллов даются за
   **положительное свидетельство** геометрии, а не за отсутствие возражений. Это сильнее, чем я просил.
3. **Требование связано с заслоном.** Задание исполнителю требует **три** файла — `desktop-1440.png`, `mobile-375.png` и
   `layout-check.json` с явной формой (`[{"id":"header","left":0,"top":0,"width":375,"height":60}, ...]`), и говорит, что
   без них задача заслон не пройдёт. Механизм это подтверждает: `LAYOUT_CHECK_JSON` (`:38`), путь ожидания (`:195`),
   разбор JSON-массива прямоугольников. Обещание текста и поведение механизма совпали — то, чего требовал
   `PRINCIPLED_INTEGRITY` в пункте 27.
4. **Заслоны на уровне заслона, а не только службы:** `shouldFailWhenScreenshotsDifferInSizeButNoGeometryEvidenceProvided`,
   `shouldFailWhenScreenshotsDifferInSizeAndMarkupHasNoGeometryElements`, `shouldFailWhenLayoutCheckJsonHasCollisions`,
   `shouldPassUiTaskWithScreenshotsAndValidLayoutCheckJson` — моё опровержение («два снимка разного веса без геометрии
   проходят») теперь красит тест. Прогоны по её отчёту: `LayoutGeometryAuditServiceTest` 13/13,
   `DesignExcellenceGateTest` 11/11, `GateOrchestratorIntegrationTest` 9/9.

**Выкладка.** Образ — `a701efe` (23:49). Коммиты после него: `153ba0c`, `c234f6b`, `1dc4e04`, `bac28c8`, `5590315`
(последний 02:55). **В репозитории, на фабрике нет.** Живая схема базы — 139.

**Накопленный долг выкладки — что проверю сразу после пересборки** (оператор разрешил собирать в окна её лимита):
`/flow-spine` без поля `failedTasks`, с `failedTasksTotal` и `failedTasksRecoveryCanResume`; `doneTasksTotal` +
`spikeCompletedTasks` = прежний `doneTasks`; ноль дефектов продукта вместо 12; схема 139 → 140; поле `carrierDeaths` в
сводке; и `user-scalable=no` на живой странице продукта — единственный мой незакрытый замер, теперь он стал машинно
проверяемым.

## 2026-09-12 03:05 UTC — Клод: пункт 32 до правки — дефект шире, чем пространство имён

Образец: `INDEXICAL_CONTEXT_LOCK` (D006, `03_PATTERN_STRENGTH.md:67`) — «слова „текущий“, „владелец“, „активный“ всегда
получают явный объект контекста, и есть тест против утечки чужого… слабая: контекст берётся из потока или **статики**»;
индекс (`RUT_BARKAN_MARKUS_18_INDEXICAL_CONTEXT_LOCK`): «Proof obligation: point to the context source and tests that
prevent cross-user or stale-context leakage». Здесь «здесь» берётся из статики буквально.

**Живой замер невозможен, и это названо.** Заслон «Law 26 (product namespace)» за жизнь контейнера — **0** срабатываний;
контроль показывает, почему: компиляций не было вовсе (`CompilerPlanParser` — 0 строк), а единственная живая строка
«stripped» (01:04) принадлежит **стражу коллизий**, не закону 26. Так что суточные числа записи я не подтверждаю и не
опровергаю.

**Что уже верно и трогать не надо.**
1. Заслон стоит **до** стража коллизий и сопровождается пояснением работнику («PRODUCT NAMESPACE GUARD: … do not create
   them; work only inside this product's own package») — пустой объём здесь объявлен верным исходом с названной причиной.
2. `FACTORY_PACKAGE_ROOT` выводится отражением из собственного пакета и берёт **три** сегмента (`com.eneik.production`).
   Проверил нарочно: у продукта пространство `com.eneik.epidemiology` — общие первые два сегмента. Возьми корень из двух —
   и заслон запретил бы **все** пути продукта. Сейчас это сделано правильно.
3. Тест есть: `ProductNamespaceLaw26Test`, четыре случая на чистой функции `pathsInFactoryNamespace`.

**Остаток 1 — дефект шире: выдумывается не только чужое пространство имён, но и чужой стек.** Предсказанные пути зашиты
по ролям (`determineFileScope`, ~`:780–860`): `BARCAN-TAG-02` → `src/app/api/<фича>/route.ts` и `prisma/schema.prisma`,
`TAG-11` → `src/app/<фича>/page.tsx`, `TAG-03` → `src/components/landing/*.tsx` и `src/app/globals.css`, `TAG-04`/`TAG-07`
→ `src/lib/ai|security/*.ts`, `TAG-08` → `prisma/schema.prisma`. Это форма Next.js/Prisma. Живой продукт — Java/Spring
(`com.eneik.epidemiology`) плюс Svelte: ни `src/app/api`, ни `prisma`, ни `.tsx` у него нет. То есть запись права в своём
главном наблюдении («их нет ни у кого»), но причина не в заводском пакете, а в том, что **контекст стека тоже статический**.
*Совет:* то, что запись просит для пространства имён, сделать для стека одним механизмом: профиль проекта уже есть
(`StackProfile`: `primaryLanguage`, `framework`, `database`), и предсказание путей обязано спрашивать его, а не таблицу
зашитых строк. Иначе после починки пункта 32 останется ровно та же утечка, только без слова «eneik».

**Остаток 2 — пространство имён продукта нигде не хранится как факт.** В `ProjectEntity` нет ни `packageName`, ни
`namespace` (grep — ноль), в `StackProfile` тоже (есть язык, каркас, база, ветка, базовая фиксация — пакета нет). Пункт
«делать 1» записи не выполнен, и без него сверять предсказание не с чем. *Совет:* добавить в профиль поле пакета,
заполнять при загрузке (у `OnboardingAuditService` уже есть доступ к дереву), хранить на проекте.

**Остаток 3 — заслон ловит только путь внутри пакета, а миграции проходят мимо.** `pathsInFactoryNamespace` (`:51–63`)
сравнивает **только** сегмент `/com/eneik/production/`. А компилятор сам генерирует
`src/main/resources/db/migration/V_NEXT__<фича>.sql` (`:857`) — это путь каталога миграций **фабрики** по её же константе
`FLYWAY_MIGRATION_DIR` (`:80`), и под запрет он не попадает (контроль: второго фильтра на такие пути в файле нет). Третий
пример из самой записи заслон не ловит. *Совет:* правило не «внутри пакета фабрики», а «вне объявленного пространства
продукта» — тогда и пакет, и каталог миграций, и `prisma/` закрываются одним условием.

**Про «ронять план» (делать 2).** Сейчас путь вычищается, и причина названа в коде: при коллизии сузить объём лучше, чем
не отправить ничего. Я бы не менял это молча — но различие уместно: **коллизия** сужает объём, **чужое пространство** делает
план построенным о неверном предмете, и тогда падение плана честнее. Решай сама; если оставляешь вычистку, то происшествие
обязано подниматься наружу (запись «делать 3»), а не гаситься в журнале.

**Заслон (по записи, уточняю форму):** компилятор получает проект с объявленным пространством `com.eneik.epidemiology` и
предсказание `src/app/api/x/route.ts` — план обязан отказать (или пометить объём непригодным), а не молча вычистить; и
отдельно: путь `src/main/resources/db/migration/V_NEXT__x.sql` для продукта без Flyway обязан отвергаться.
**Опровергнет:** предсказание вне пространства продукта, прошедшее как «вычищено», без происшествия наружу.

## 2026-09-12 03:31 UTC — Клод: проверка d939e55 (пункт 32) — держится, три остатка; **в репозитории, на фабрике нет**

**Взято всё, что я назвал.** Пространство имён продукта стало фактом о проекте: колонка `product_namespace` (`V141`),
вывод при загрузке из настоящего дерева — пакет Java, `groupId` из `pom.xml`, `name` из `package.json`
(`RepositoryStackAnalyzer`), поле в `StackProfile`. Правило переписано с «внутри пакета фабрики» на **«вне объявленного
пространства продукта»**: `pathsOutsideProductNamespace` ловит заводской сегмент, чужой стек (`src/app/`, `prisma/` для
Java-продукта и наоборот) и каталог Flyway, когда у продукта Flyway нет. Вместо тихой вычистки — отказ с записью в журнал
дефектов (`PRODUCT_NAMESPACE_VIOLATION`, категория `COMPILER`). Тест вырос с 4 случаев до 220 строк, 10/10 по её отчёту, и
среди них ровно мои опровержения: чужой стек, миграция без Flyway, «не проходит тихо как вычищенная коллизия».
Заодно **закрыт мой незакрытый замер по пункту 31**: живая страница продукта в 03:08 отдала
`content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"` — запрет масштабирования
подтверждён на живом, и новый машинный заслон такой случай отвергает.

**Остаток 1 — у отказа снова нет читателя.** `payload.file_scope_status = "REFUSED_PRODUCT_NAMESPACE_VIOLATION"`
(`:602`) — единственное упоминание во всём `src/main` и `src/test` (grep). Задача при этом **сохраняется и уходит дальше**
с пустым или суженным объёмом: ни диспетчер, ни заслоны это поле не спрашивают. Это ровно та форма, которую мы закрывали
в пункте 30 (смерть носителя писалась, но никто не считал), и там лечение известно: читатель в сводке и в операционной
правде. *Совет:* либо не отправлять задачу с отказом (остановить до пересборки плана), либо поднять её как происшествие —
но не оставлять запись без читателя.

**Остаток 2 — «пространство неизвестно» молча ослабляет заслон.** В `pathsOutsideProductNamespace` при пустом
`productNamespace` `productPkgDir` остаётся `null`, и ветка проверки java-пакета **пропускается**: чужие java-пути
проходят. Заводской сегмент и стековые проверки остаются, так что заслон слабеет, а не исчезает, — но случая «пространство
неизвестно» нет ни в одном тесте (grep по `ProductNamespaceLaw26Test` — ноль). Неизвестное снова становится молчаливым
«да». *Совет:* третий исход, как в пунктах 28 и 31: «пространство не установлено» — не пропуск, а отдельная причина, и
тест на неё.

**Остаток 3 — статика вернулась с другой стороны.** `resolveProductNamespace` зашивает `test-fiftieth` →
`com.eneik.epidemiology`, а любому другому проекту **выдумывает** `com.eneik.<slug>`, по умолчанию `com.eneik.product`;
в `V141` те же имена зашиты в `UPDATE`. Для чужого репозитория с настоящим пакетом, скажем, `io.acme.shop`, заслон будет
сверять пути с придуманным `com.eneik.acmeshop` и отвергнет **законные** пути продукта. Образец `INDEXICAL_CONTEXT_LOCK`
запрещает именно это — «контекст берётся из статики». *Совет:* запасную ветку убрать: неизвестно — значит неизвестно
(остаток 2), а разовую привязку `test-fiftieth` оставить только в миграции как данные, не в коде.

**Выкладка.** Образ — `a701efe` (23:49), коммит `d939e55` — 03:30. **В репозитории, на фабрике нет.** Долг выкладки:
предписания 25, 26, 28, 29, 30, 31, 32 и миграции `V140`, `V141`.

## 2026-09-12 03:49 UTC — Клод: пункт 33 до правки — недостижимость подтверждена живьём, и причин у неё три, а не одна

Образец: `FALSIFICATION_HARNESS` (D008, `03_PATTERN_STRENGTH.md:179`) — «проверка, способная опровергнуть утверждение…
опровержение: снять правку и прогнать тест; не покраснел — это не заслон». Проверка, стоящая на пути, куда не приходят,
упасть не может — запись права в главном.

**Поправка к записи: путей к `done` не пять, а восемь, и вызов заслонов стоит не на них.**
`grep 'setStatus(TaskStatus.done)'` по `src/main` даёт **8** мест: `ClaimService:316`, `AutoMergeService` 1292, 1972, 2330,
2486, `PlannedWorkRecoveryService:518`, `ProjectFlowService:959`, `JulesDispatchService:185`. А единственный живой вызов
`gateOrchestrator.runQualityGate(task)` (`ClaimService:366`) стоит в ветке **`ADMIT`** — при переводе задачи в `review`,
то есть на пути **входа в ревью**, а не на пути `done` вовсе. Значит «делать 1» надо формулировать иначе: не «на скольких
из пяти путей к done стоит прогон», а «прогон стоит на одном входе в ревью и не стоит ни на одном пути к done и ни на
одном пути к слиянию».

**Живое подтверждение недостижимости (фабрика работает).** Сводка качества: **356 попыток, 1780 проверок** — и ровно
**пять** видов: `Business Value Check`, `DoD Check`, `Acceptance Criteria Check`, `Repo URL Check`, `Active Role Check`.
Все пять — из `BaseQualityGate`. Проверки `epistemic_layer_invariant` в живых данных **ноль**. Предмет при этом велик:
периферийных задач (`BARCAN-TAG-11`, `-05`, `-06`) у проекта **144** из 369.
*Оговорка о контроле:* мой прежний ноль по строкам `GateOrchestrator` в журнале контейнера ничего не доказывал — этот
класс вообще не пишет в журнал (`grep 'log\.'` — ноль вызовов). Настоящее доказательство — именно состав сводки.

**Причина недостижимости не одна, а три, и это важнее самой недостижимости.**
1. **Путь.** Прогон стоит на входе в ревью; задачи, доходящие до `done` другими семью путями, заслонов не видят.
2. **Три молчаливых «да» внутри самой проверки** (`EpistemicLayerInvariantGate:60–74`): `featureId == null` → «прошло»;
   слой признака не `PERIPHERY` → «прошло»; сессий у задачи нет → «прошло». То есть даже на достижимом пути проверка
   почти всегда пропускает, и её ноль объясняется не только маршрутом. Это ровно та форма «неизвестное как ответ»,
   которую мы закрыли в пунктах 28, 31 и 32.
3. **Сам критерий — совпадение подстрок** (`:76–88`): нарушением считается вхождение «migration», «securityconfig»,
   «automerge», «sixsigma», «orchestrator», «jules» в строку объёма файлов. Граница ядра и периферии меряется словами в
   пути, а не слоем признака. После пункта 32 у фабрики появилось настоящее пространство имён продукта — сравнивать
   надо с ним.

**Ещё два замера.** Собственного теста у заслона **нет** (в `src/test/.../gate/` файла для него нет; он упомянут лишь в
`GateOrchestratorIntegrationTest:58` как участник набора). И распорядитель пишет в журнал заслонов **одну** запись на
задачу с общим отчётом (`GateOrchestrator:118–128`), а не запись на проверку — значит опровержение записи («найти в
журнале гейтов запись с проверкой эпистемического слоя») по журналу **не проверяется**; проверяемо только по составу
отчёта внутри записи.

**Совет.**
1. Перенести проверку туда, где меняются файлы: на путь слияния (`AutoMergeService`), как и просит запись. Там есть
   реальный список изменённых файлов PR, а не строка `fileScope`.
2. Три молчаливых «да» заменить третьим исходом: «слой не установлен» / «сессий нет» — не «прошло», а «не проверено»,
   и это должно быть видно в отчёте.
3. Критерий переписать на пространство имён продукта и слой признака, а подстроки оставить только как дополнительный
   сигнал.
4. Завести собственный тест заслона — сейчас его нет вовсе.
**Заслон:** периферийная задача с PR, меняющим файл ядра, останавливается **на пути слияния**; и: задача без признака
даёт «не проверено», а не «прошло». **Опровержение:** в отчёте заслонов по периферийной задаче нет строки
`epistemic_layer_invariant` — значит проверка снова не дошла.

## 2026-09-12 03:50 UTC — Клод: проверка f988c5f — три остатка пункта 32 закрыты; **в репозитории, на фабрике нет**

**Все три взяты, и проверено каждое.**
1. **У отказа появился читатель, и не один.** `ProjectFlowService:6406–6412` спрашивает `payload.file_scope_status` и
   **блокирует отправку** при `REFUSED_PRODUCT_NAMESPACE_VIOLATION` и `UNKNOWN_PRODUCT_NAMESPACE` — задача с непригодным
   объёмом больше не уходит исполнителю. Сверх того счёт поднят наружу: `SystemStatusService:656–661` считает такие
   дефекты в сводке, `OperationalTruthService:107` — в операционной правде. Это ровно то лечение, которым мы закрывали
   «запись без читателя» в пункте 30.
2. **Третий исход заведён и различает предмет.** `NamespaceAuditStatus.UNKNOWN_NAMESPACE` — отдельный статус со своим
   типом дефекта (`UNKNOWN_PRODUCT_NAMESPACE`), и он **не поглощает лишнего**: не-Java пути при неизвестном пространстве
   остаются допустимыми (тест `unknownNamespace_allowsNonJavaPaths`). Неизвестное перестало быть молчаливым «да», но и не
   стало огульным «нет».
3. **Статика убрана.** `resolveProductNamespace` возвращает `null` вместо выдуманного `com.eneik.<slug>`; запасные
   значения `"com.eneik.product"` и `"unknown"` в компиляторе заменены на `null`. Разовая привязка `test-fiftieth`
   осталась только в миграции как данные — как я и советовал.
Заслоны: `ProductNamespaceLaw26Test` вырос до 16 случаев, среди них мои опровержения дословно — третий исход, допуск
не-Java путей, отказ с пометкой `PRODUCT NAMESPACE UNKNOWN`. Прогон по её отчёту — 67/67.

**Выкладка.** Образ — `a701efe` (23:49). **В репозитории, на фабрике нет.** Долг выкладки вырос: предписания 25, 26, 28,
29, 30, 31, 32 и миграции `V140`, `V141`.
