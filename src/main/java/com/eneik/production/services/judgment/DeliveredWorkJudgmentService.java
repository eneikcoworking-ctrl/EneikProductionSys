...
public class DeliveredWorkJudgmentService {
    ...
    SelectedEvidence evidenceForCriteria(String diff, String criteria) {
        return evidenceForCriteria(diff, criteria, diffCharLimit);
    }

    SelectedEvidence evidenceForCriteria(String diff, String criteria, int limit) {
        CriteriaEvidenceSelector.SelectedEvidence selected =
                CriteriaEvidenceSelector.select(diff, criteria, limit);
        return new SelectedEvidence(selected.text(), selected.omitted());
    }
    ...
    private String prompt(TaskEntity task, String prUrl, String diff) {
        String titleBlock = "TASK TITLE\n" + (task.getTitle() == null ? "" : task.getTitle()) + "\n\n";
        String descBlock = "WHAT THIS TASK WAS ASKED TO DO\n"
                + cap(task.getDescription() == null ? "(no description recorded)" : task.getDescription(),
                     DESCRIPTION_CHAR_LIMIT) + "\n\n";
        String criteriaBlock = "ACCEPTANCE CRITERIA THIS TASK CARRIED\n"
                + cap(task.getAcceptanceCriteria(), 4_000) + "\n\n";
        String prBlock = "MERGED PULL REQUEST\n" + prUrl;
        String repoBlock = repositoryState(task);

        String contextHeader = titleBlock + descBlock + criteriaBlock + prBlock + repoBlock;

        // Law 17: Select diff by criteria within the remaining budget, never mechanically truncate by position.
        // Target 36_000 chars leaves headroom for the system instruction in JudgmentAgentClient.
        int targetUserPromptLimit = 36_000;
        int estimatedWarningLen = 600;
        int diffBudget = Math.max(5_000, targetUserPromptLimit - contextHeader.length() - estimatedWarningLen);

        SelectedEvidence evidence = evidenceForCriteria(diff, task.getAcceptanceCriteria(), diffBudget);
        ...
        String warning = evidence.omitted().isEmpty()
                ...
                + "criteria above, not by position, and it is " + bounded.length() + " of " + (diff == null ? 0 : diff.length())
    ...
    private static final int REPOSITORY_PATHS_SHOWN = 50;
